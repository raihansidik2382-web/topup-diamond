    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <select name="game_id" id="game_id" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
                ...
                @foreach ($games as $game)
                    <option value="{{ $game->id }}" {{ old('game_id', $product->game_id) == $game->id ? 'selected' : '' }}><img src="{{ asset('images/games/' . $game->icon) }}" alt="{{ $game->name }}" class="inline size-5 object-contain align-middle mr-1"> {{ $game->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="currency" id="currency" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                ...
                <option value="USD" {{ old('currency', $product->currency) == 'USD' ? 'selected' : '' }}>USD (Dolar)</option>
            </select>
