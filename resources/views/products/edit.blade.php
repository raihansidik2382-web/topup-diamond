...
@section('content')
    <h1 class="text-2xl font-black uppercase tracking-[0.15em] mb-6">Edit Produk Topup</h1>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <select name="game_id" id="game_id" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30" required>
                ...
                @foreach ($games as $game)
                    <option value="{{ $game->id }}" {{ old('game_id', $product->game_id) == $game->id ? 'selected' : '' }} class="bg-navy-dark">{{ $game->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] placeholder-muted focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30" required>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="currency_amount" id="currency_amount" value="{{ old('currency_amount', $product->currency_amount) }}" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] placeholder-muted focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30" required>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="price" id="price" value="{{ old('price', $product->price) }}" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] placeholder-muted focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30" required>
        ...
        <div class="mb-4">
            ...
            <select name="currency" id="currency" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30">
                <option value="IDR" {{ old('currency', $product->currency) == 'IDR' ? 'selected' : '' }} class="bg-navy-dark">IDR (Rupiah)</option>
                <option value="USD" {{ old('currency', $product->currency) == 'USD' ? 'selected' : '' }} class="bg-navy-dark">USD (Dolar)</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" class="block w-full rounded-lg bg-navy-light border border-white/5 px-3 py-2 text-sm text-[#f5f5f5] placeholder-muted focus:outline-none focus:border-orange-accent focus:ring-1 focus:ring-orange-accent/30">{{ old('description', $product->description) }}</textarea>
            @error('description') <p class="mt-1 text-sm text-red-400">{{ $message }}</p> @enderror
        </div>

        <div class="mb-6">
            <label class="flex items-center gap-2">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="rounded border-white/5 bg-navy-light text-orange-accent focus:ring-orange-accent/30">
