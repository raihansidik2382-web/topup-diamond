...
@section('content')
    <h1 class="text-2xl font-semibold mb-6">Tambah Produk Topup</h1>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="100 Diamonds" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
            @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label for="currency_amount" class="block text-sm font-medium text-gray-700 mb-1">Jumlah Mata Uang Game</label>
            <input type="number" name="currency_amount" id="currency_amount" value="{{ old('currency_amount') }}" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">{{ old('description') }}</textarea>
