...
@section('content')
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('orders.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-500">Tambah Pesanan</a>
    </div>
                ...
                @forelse ($orders as $order)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <form action="{{ route('orders.destroy', $order) }}" method="POST" class="inline ml-3" onsubmit="return confirm('Yakin ingin menghapus pesanan ini?')">
                                @csrf
                                @method('DELETE')
