...
@php $slideId = 'hero-slider-' . md5(serialize($slides)); @endphp
    ...
    <div class="relative z-10 text-center px-4 md:px-6">
        ...
        @if (count($slides) > 1)
            <div class="slide-dots mt-6 flex items-center justify-center gap-2">
                @foreach ($slides as $i => $image)
                    <button data-index="{{ $i }}" class="size-2 rounded-full transition-all duration-300 {{ $i === 0 ? 'bg-orange-accent w-6' : 'bg-white/30 hover:bg-white/50' }}"></button>
                @endforeach
            </div>
        @endif
    </div>
...
@if (count($slides) > 1)
    <script>
        (function() {
            var c = document.getElementById('{{ $slideId }}');
            if (!c) return;
            var s = c.querySelectorAll('.slide');
            var d = c.querySelectorAll('.slide-dots button');
            ...
            d.forEach(function(el) { el.addEventListener('click', function() { go(parseInt(this.getAttribute('data-index'))); }); });

            if (t > 1) setInterval(function() { go((cur + 1) % t); }, 4500);
        })();
    </script>
@endif